  ...
  try {
    ...
    if (action === "upsertPlan") {
      ...
      const payload = {
        ...
        quotaUnit: data?.quotaUnit || "GB", // MB or GB
        uploadKbps: Number(data?.uploadKbps || 0),
        downloadKbps: Number(data?.downloadKbps || 0),
        duration: Number(data?.duration || 0),
        durationType: data?.durationType || "days", // hours/days/months
        updatedAt: new Date(),
    ...
    if (action === "createCardsBatch") {
      ...
      for (let i=0;i<count;i++) {
        batch.push({
          ...
          status: "new", // new | active | used | expired
          createdAt: new Date()
        });
      }
